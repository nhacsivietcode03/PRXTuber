...
const ArtistListScreen = ({ route, navigation }) => {
  ...
  const handleArtistPress = async (artist) => {
    if (features) {
      try {
        Toast.show({ type: 'info', text1: 'Loading Playlist...', text2: `Fetching tracks for ${artist.title || artist.name}` });
        
        let tracks = [];
        if (artist.type === 'playlist' && artist.originalId) {
          tracks = await getTracksByPlaylist(artist.originalId);
        } else if (artist.genre) {
          tracks = await getTracksByGenre(artist.genre, 30, 'popularity_month');
        }
  ...
  const renderArtistItem = ({ item, index }) => {
    ...
    return (
      ...
      >
        ...
        {features && item.tracks && (
          <Text style={[styles.artistName, {color: colors.textSecondary, fontSize: 10, marginTop: 2}]} numberOfLines={1}>
